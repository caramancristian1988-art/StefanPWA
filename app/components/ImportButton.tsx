...
export default function ImportButton({ entity, className }: Props) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const res = await fetch(`/api/import?entity=${entity}`, {
        method: "POST",
        body: buf,
        headers: { "Content-Type": file.name.endsWith(".csv") ? "text/csv" : "application/octet-stream" },
      });
              ...
              <>
                <h2 className="mb-1 text-base font-semibold">
                  {result.imported === result.total
                    ...
                    : `Import parțial — ${result.imported} din ${result.total} rânduri importate`}
                </h2>
                {result.failed.length > 0 && (
                  <>
                    <p className="mb-2 mt-3 text-xs font-medium uppercase tracking-wide text-ink-soft">
                      Rânduri cu erori ({result.failed.length})
                    </p>
