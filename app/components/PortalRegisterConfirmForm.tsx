...
import { confirmClientRegistration, type ClientAuthState } from "@/app/actions/client-auth";
...
export default function PortalRegisterConfirmForm({
  meterSeries,
  email,
}: {
  meterSeries: string;
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3">
      <input type="hidden" name="meterSeries" value={meterSeries} />
      ...
      >
        {pending ? "Se procesează…" : "Activează contul"}
      </button>
    </form>
  );
}
